...
// rooms[Raum][Variante]
  if (urlLine.indexOf("/onTime") >= 0)
  {
    ...
    v0 = getStringPartByNr(urlLine, '/', 2).toInt(); // Raum
    v1 = getStringPartByNr(urlLine, '/', 3).toInt(); // onTime
    rooms[v0][curr_pin].onTime = v1;
    Serial.println("onTime: " + String(rooms[v0][curr_pin].onTime));
    sprintf(buffer, "onTime%d/%d", v0, curr_pin);
    Serial.println("buffer: " + String(buffer));
    preferences_light.putUShort(buffer, rooms[v0][curr_pin].onTime);
  }
  if (urlLine.indexOf("/offTime") >= 0)
  {
    Serial.println("Message: " + String(urlLine));
    v0 = getStringPartByNr(urlLine, '/', 2).toInt(); // Raum
    v1 = getStringPartByNr(urlLine, '/', 3).toInt(); // offTime
    rooms[v0][curr_pin].offTime = v1;
    sprintf(buffer, "offTime%d/%d", v0, curr_pin);
    Serial.println("buffer: " + String(buffer));
    preferences_light.putUShort(buffer, rooms[v0][curr_pin].offTime);
  }
