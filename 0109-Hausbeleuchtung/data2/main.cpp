...
uint8_t pins[max_pins] ={0, 0, LED_PIN_HOUSELIGHT, LED_PIN_RUNNINGLIGHT, 0};
...
{
  uint8_t roomnbr = 0;
  String paramsString = String(rooms[roomnbr].onTime) + "/" + String(rooms[roomnbr].offTime) + "/" + String(rooms[roomnbr].brightness) + "/" + String(baseTime_house);
  return paramsString;
}
