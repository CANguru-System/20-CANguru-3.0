...
void Scan4Slaves()
{
  ...
  for (uint8_t trial = 0; trial < scanTrial; trial++)
  {
    ...
    char chs[50];
    sprintf(chs, "Scan# %d -/- %d slave(s) found of %d!", trial + 1, tmpscanResults, wantedscanResults);
    displayLCD(chs);
...
*/
/**************************************************************************/
void printStruct(uint s)
{
  Serial.println(String(s) + ": " + String(slaveInfo[s].intValue));
}
