    ...
    public partial class Form1 : Form
    {
                            ...
                            switch (content[1])
                            {
                                    ...
                                    if (listFile)
                                    {
                                        ...
                                        if (content[0x04] == 6)
                                        {
                                            ...
                                            // 0 ist die Gerätebeschreibung (Paket 0)
                                            {
                                                ...
                                                //
                                                outputlistFile.WriteLine("\tIP-Adresse: " + makeIPAddress(CANguruLFDecoderNbr));
                                                //
                                                // Anzahl der folgenden Messwerte aus Zeile 2, Position 6
                                ...
                                case 0x65:
                                    ...
                                    for (byte c = 0; c < CANguruArrFilled; c++)
                                    {
                                        ...
                                        if (decoders.Contains(ip) == true)
                                        {
                                            // decoders ist die Liste der Decoder, die in der letzten Sitzung angemeldet waren.
                                            // Der gerade angemeldete Decoder wird von dieser Liste gestrichen; übrig bleiben die, die nicht wieder gefunden wurden.
                                            decoders.Remove(ip);
                                        }
                                    }
                                    break;
        ...
        private void proc_beenden()
        {
            ...
            for (byte d = 0; d < decs; d++)
            {
                ...
                // XXXX
                //ini.AddSection("Decoder").AddKey(String.Concat("Decoder", String.Format("{0:D03}", d))).Value = CANElemente.Text + " / " + makeIPAddress(d);
                ini.AddSection("Decoder").AddKey(String.Concat("Decoder", String.Format("{0:D03}", d))).Value = makeIPAddress(d);
            }
